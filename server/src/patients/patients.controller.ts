...
export class PatientsController {
  ...
  @ApiQuery({ name: 'sortBy', required: false, enum: ['id', 'clinic_id', 'first_name', 'last_name', 'date_of_birth'] })
  ...
  @Get('clinic-name')
  @ApiOperation({ summary: 'Get patients by clinic name with optional sorting' })
  @ApiQuery({ name: 'name', description: 'Clinic name', example: 'Salve Fertility' })
  @ApiQuery({ name: 'sortBy', required: false, enum: ['id', 'clinic_id', 'first_name', 'last_name', 'date_of_birth'] })
  @ApiQuery({ name: 'sortOrder', required: false, enum: ['asc', 'desc'] })
  @ApiResponse({ 
    status: 200, 
